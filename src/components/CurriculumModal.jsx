...
// ─── Mühendislik Fakültesi: tek JSON'dan okunacak bölümler ───────────────────
// public/metu_eng_faculty_mufredat.json scraper çıktısı kullanılır.
// prog_id → JSON içindeki bolumler[].prog_id ile eşleşir.
const ENG_FACULTY_FILE = "/metu_eng_faculty_mufredat.json";
...
// ─── Diğer bölümler: ayrı dosyalardan okunur ─────────────────────────────────
const OTHER_CURRICULA = [
  ...
  { label: "MSE — Mat. ve Fen Bilimleri Eğitimi",       file: "/scraper/mufredat_output/mse_mufredat.json",   coursesFile: null },
  ...
  // EECE ve CENG ders havuzlarını ayrı dosyadan okumaya devam edelim
  { label: "EECE — Elektrik & Bilg. Müh. (EECE)",       file: "/scraper/mufredat_output/eece_mufredat.json",  coursesFile: null },
];

// ─── Yeni scraper çıktısını CurriculumModal yapısına dönüştür ─────────────────
const YIL_ADI_MAP = { 1: "1. Yıl", 2: "2. Yıl", 3: "3. Yıl", 4: "4. Yıl" };
...
function normalizeDersler(dersler) {
  return (dersler || [])
    ...
    .map(d => {
      const out = { ...d };
      // odtu_kredi → her ikisini de tut (backward compat)
      ...
      // secenekler varsa özyinelemeli temizle
      if (out.secenekler) out.secenekler = normalizeDersler(out.secenekler);
      return out;
    });
}

function convertEngBolum(bolum) {
  // Yeni scraper: bolum.mufredat = [ { yil, yariyil, yariyil_adi, dersler[] } ]
  // Hedef:        { mufredat: [ { yil, yil_adi, yariyillar: [ { yariyil, yariyil_adi, dersler[] } ] } ] }
  const yariyilList = bolum.mufredat || [];

  // yariyilleri yıla göre grupla
  const yilMap = {};
  for (const y of yariyilList) {
    const yilNo = y.yil ?? Math.ceil(y.yariyil / 2);
    if (!yilMap[yilNo]) yilMap[yilNo] = [];
    yilMap[yilNo].push({
      yariyil:     y.yariyil,
      yariyil_adi: y.yariyil_adi || `${y.yariyil}. Dönem`,
  ...
  const mufredat = Object.keys(yilMap)
    ...
    .map(yilNo => ({
      ...
      yariyillar: yilMap[yilNo],
    }));

  return { mufredat };
}

// ─── Mevcut (eski) JSON yapısını normalize et ─────────────────────────────────
function normalizeLegacyCurriculum(raw) {
  const list = Array.isArray(raw?.mufredat)   ? raw.mufredat
...
// ─── Tüm bölüm listesi (seçim kutusu için) ───────────────────────────────────
// Tip: { label, isEng, prog_id? (eng), file? (other), coursesFile? }
function buildCurriculaList() {
  const eng   = ENG_DEPTS.map(d  => ({ ...d, isEng: true  }));
  const other = OTHER_CURRICULA.map(d => ({ ...d, isEng: false }));
  return [...eng, ...other].sort((a, b) => a.label.localeCompare(b.label, "tr"));
}
const ALL_CURRICULA = buildCurriculaList();

// ─── CENG ders havuzu (eski mantık, sadece CENG için) ────────────────────────
const CATEGORY_TABS = [
...
function CourseRow({ ders, catalogEntry, isLast, tr, done, onToggle, viewFilter }) {
  ...
  return (
    <div style={{ borderBottom: isLast ? "none" : "1px solid #f5f0ec" }}>
      <div onClick={e => { if (e.target.dataset.toggle) return; if (!catalogEntry) return; if (hasSections) setOpen(v=>!v); else if (url) window.open(url,"_blank"); }}
        style={{ padding:"7px 12px", display:"flex", alignItems:"center", gap:9,
          background: isDone?"#f0fdf4":open?"#fdf8f5":(catalogEntry?"#fff":"#fafafa"),
          opacity: catalogEntry?1:0.55, cursor: catalogEntry?"pointer":"default", transition:"background .12s" }}>
        <button data-toggle="1"
          ...
          style={{ width:18, height:18, borderRadius:5, flexShrink:0, border:isDone?"none":"2px solid #d1d5db",
            background:isDone?"#22c55e":"transparent", cursor:"pointer", display:"flex", alignItems:"center",
            justifyContent:"center", fontSize:"0.65rem", color:"#fff", transition:"all .15s", padding:0, outline:"none" }}>
          {isDone ? "✓" : ""}
        </button>
        <div style={{ width:7, height:7, borderRadius:"50%", flexShrink:0, background:catalogEntry?"#22c55e":"#d1d5db" }} />
        <span style={{ fontSize:"0.77rem", fontWeight:700, color:isDone?"#15803d":"#7a1f2b", minWidth:68, flexShrink:0,
          textDecoration:isDone?"line-through":"none", opacity:isDone?0.7:1 }}>{ders.kod}</span>
        <span style={{ fontSize:"0.77rem", color:isDone?"#888":"#333", flex:1, lineHeight:1.4,
          textDecoration:isDone?"line-through":"none" }}>{ders.ad}</span>
        ...
        {catalogEntry && !isDone && (
          hasSections
            ? <span style={{ fontSize:"0.7rem", color:"#aaa", flexShrink:0, display:"inline-block",
        ...
        <div style={{ background:"#fdf8f5", borderTop:"1px solid #f0ece8", padding:"8px 12px 10px 40px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
            {sections.map((sec,i) => {
              ...
              return (
                <div key={i} style={{ fontSize:"0.73rem", padding:"5px 8px", borderRadius:6,
                  background:"#fff", border:"1px solid #e5e0da", display:"flex", gap:10,
...
function SecmeliGrupRow({ grup, courses, isLast, tr, done, onToggle, viewFilter }) {
  ...
  return (
    <div style={{ borderBottom: isLast?"none":"1px solid #f5f0ec" }}>
      <div onClick={() => setOpen(v=>!v)} style={{ padding:"7px 12px", display:"flex", alignItems:"center",
        gap:8, background:open?"#fdf5e6":"#fffdf8", cursor:"pointer" }}>
        <div style={{ width:18, height:18, flexShrink:0 }} />
        <div style={{ width:7, height:7, borderRadius:"50%", background:"#f59e0b", flexShrink:0 }} />
        <span style={{ fontSize:"0.75rem", fontWeight:600, color:"#92400e", flex:1 }}>{grup.secmeli_grup}</span>
        <span style={{ fontSize:"0.68rem", color:"#aaa", display:"inline-block",
          transition:"transform .15s", transform:open?"rotate(180deg)":"none" }}>▼</span>
      </div>
      {open && (
...
function SlotRow({ slot, isLast, viewFilter }) {
  ...
  return (
    <div style={{ borderBottom:isLast?"none":"1px solid #f5f0ec", padding:"7px 12px",
      display:"flex", alignItems:"center", gap:9, background:"#f9f9f9", opacity:0.6 }}>
      <div style={{ width:18, height:18, flexShrink:0 }} />
...
export default function CurriculumModal({ lang, courses, onApplyToScheduler, onClose }) {
  ...
  const [engFacultyData,  setEngFacultyData]  = useState(null);  // tüm eng fakülte JSON'ı
  const [curriculum,      setCurriculum]      = useState(null);  // aktif bölüm mufredat
  ...
  const doneKey = selectedDept.isEng
    ? `eng_${selectedDept.prog_id}`
    : selectedDept.file;
  ...
  // Mühendislik fakültesi JSON'ını bir kez yükle
  useEffect(() => {
    if (engFacultyData) return;
    fetch(ENG_FACULTY_FILE)
      .then(r => r.json())
      .then(setEngFacultyData)
      .catch(() => {}); // sessizce başarısız ol, bölüm bazlı hata göster
  }, []);

  // Bölüm değişince müfredatı yükle
  useEffect(() => {
    setLoading(true);
    setError(null);
    setCurriculum(null);
    ...
    if (selectedDept.isEng) {
      // Mühendislik fakültesi JSON'ından ilgili bölümü al
      const loadFromEng = (data) => {
        ...
        if (!bolum || bolum.hata) {
          ...
          setCurriculum(convertEngBolum(bolum));
        }
        setLoading(false);
      };

      if (engFacultyData) {
        loadFromEng(engFacultyData);
      } else {
        fetch(ENG_FACULTY_FILE)
          .then(r => r.json())
          .then(data => { setEngFacultyData(data); loadFromEng(data); })
          .catch(() => {
      ...
      // Eski bölümler — ayrı dosyadan yükle
      Promise.all([
  ...
  // Dropdown için separator ekle
  const renderOptions = () => {
    const engGroup  = ALL_CURRICULA.filter(d => d.isEng);
    const otherGroup= ALL_CURRICULA.filter(d => !d.isEng);
    return (
      <>
        <optgroup label={tr ? "── Mühendislik Fakültesi ──" : "── Faculty of Engineering ──"}>
          {engGroup.map(d => (
            <option key={`eng_${d.prog_id}`} value={`eng_${d.prog_id}`}>{d.label}</option>
          ))}
        </optgroup>
        <optgroup label={tr ? "── Diğer Bölümler ──" : "── Other Departments ──"}>
          {otherGroup.map(d => (
  ...
  const selectValue = selectedDept.isEng ? `eng_${selectedDept.prog_id}` : selectedDept.file;

  const handleSelectChange = (e) => {
    const val = e.target.value;
    if (val.startsWith("eng_")) {
      const prog_id = parseInt(val.replace("eng_", ""), 10);
      setSelectedDept(ALL_CURRICULA.find(d => d.isEng && d.prog_id === prog_id));
    } else {
      setSelectedDept(ALL_CURRICULA.find(d => !d.isEng && d.file === val));
    }
  };

  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:1000, background:"rgba(0,0,0,0.45)",
      display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:16, width:"100%",
        maxWidth:580, boxShadow:"0 8px 40px rgba(0,0,0,0.18)", overflow:"hidden", display:"flex",
        flexDirection:"column", maxHeight:"92vh" }}>

        {/* Başlık */}
        <div style={{ padding:"14px 20px", borderBottom:"1px solid #f0ece8", display:"flex",
        ...
        <div style={{ padding:"16px 20px", overflowY:"auto", flex:1 }}>
          ...
          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:"0.75rem", fontWeight:600, color:"#888", marginBottom:6,
              textTransform:"uppercase", letterSpacing:"0.05em" }}>
              {tr ? "Bölüm" : "Department"}
            </div>
            <select value={selectValue} onChange={handleSelectChange} style={{ width:"100%",
              padding:"8px 12px", borderRadius:8, fontSize:"0.85rem", border:"2px solid #e5e0da",
              background:"#fff", color:"#333", cursor:"pointer", fontWeight:600, outline:"none",
              appearance:"none",
          ...
          {mufredat.length > 0 && (<>
            {/* İlerleme kartı */}
            <div style={{ background:"linear-gradient(135deg, #7a1f2b 0%, #a33040 100%)", borderRadius:10,
              padding:"10px 14px", marginBottom:14, color:"#fff" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  ...
                  {totalDone > 0 && (
                    <button onClick={clearDone} style={{ fontSize:"0.62rem", padding:"1px 6px", borderRadius:99,
                      border:"1px solid rgba(255,255,255,0.4)", background:"transparent",
                      color:"rgba(255,255,255,0.75)", cursor:"pointer", fontWeight:600 }}>
                      {tr ? "Temizle" : "Clear"}
                    </button>
                  )}
                </div>
              ...
              <div style={{ height:5, borderRadius:99, background:"rgba(255,255,255,0.2)", overflow:"hidden", marginBottom:8 }}>
                <div style={{ height:"100%", borderRadius:99, background:"rgba(255,255,255,0.9)",
                  ...
                  <div key={ys.yil} style={{ display:"flex", alignItems:"center", gap:8 }}>
                    ...
                    <div style={{ flex:1, height:3, borderRadius:99, background:"rgba(255,255,255,0.2)", overflow:"hidden" }}>
                      <div style={{ height:"100%", borderRadius:99,
                        ...
                        width: ys.total?`${(ys.done/ys.total)*100}%`:"0%", transition:"width .5s" }} />
                    </div>
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:"0.75rem", fontWeight:600, color:"#888", marginBottom:6,
                textTransform:"uppercase", letterSpacing:"0.05em" }}>{tr?"Yıl":"Year"}</div>
                ...
                {mufredat.map(y => {
                  ...
                  return (
                    <button key={y.yil} onClick={() => setSelectedYil(selectedYil===y.yil?null:y.yil)} style={{
                      padding:"6px 13px", borderRadius:8, fontSize:"0.82rem", cursor:"pointer",
                      fontWeight:600, transition:"all .15s",
                      border: selectedYil===y.yil?"2px solid #7a1f2b":"2px solid #e5e0da",
                      background: selectedYil===y.yil?"#7a1f2b":"#fff",
                      color: selectedYil===y.yil?"#fff":"#333", position:"relative" }}>
                      {yilLabel(y)}
                      {isComplete && <span style={{ position:"absolute", top:-5, right:-5,
                        fontSize:"0.65rem", background:"#22c55e", color:"#fff",
                        borderRadius:99, padding:"1px 4px", fontWeight:700 }}>✓</span>}
                    </button>
                  );
                })}
              </div>
            ...
            {selectedYil && yilData && (
              <div style={{ marginBottom:12 }}>
                <div style={{ fontSize:"0.75rem", fontWeight:600, color:"#888", marginBottom:6,
                  textTransform:"uppercase", letterSpacing:"0.05em" }}>{tr?"Dönem":"Semester"}</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
                ...
                <div style={{ display:"inline-flex", borderRadius:8, overflow:"hidden", border:"1px solid #e5e0da" }}>
                  {VIEW_FILTERS.map(f => (
                    <button key={f.key} onClick={() => setViewFilter(f.key)} style={{
                      padding:"5px 12px", fontSize:"0.77rem", fontWeight:600, border:"none",
                      ...
                      {f.key==="done" && done.size>0 && (
                        <span style={{ marginLeft:5, fontSize:"0.68rem",
                          background: viewFilter===f.key?"rgba(255,255,255,0.25)":"#f0fdf4",
                          color: viewFilter===f.key?"#fff":"#15803d", borderRadius:99, padding:"1px 5px" }}>
                ...
                {done.size > 0 && (
                  confirmClear ? (
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      ...
                      <button onClick={clearDone} style={{ fontSize:"0.73rem", padding:"4px 10px", borderRadius:7,
                        border:"none", background:"#dc2626", color:"#fff", cursor:"pointer", fontWeight:700 }}>
                        {tr?"Evet":"Yes"}</button>
                      <button onClick={() => setConfirmClear(false)} style={{ fontSize:"0.73rem", padding:"4px 10px",
                        borderRadius:7, border:"1px solid #e5e0da", background:"#fff", color:"#555",
                        cursor:"pointer", fontWeight:600 }}>{tr?"Hayır":"No"}</button>
                    </div>
                  ) : (
                    <button onClick={() => setConfirmClear(true)} style={{ fontSize:"0.75rem", padding:"5px 12px",
                      borderRadius:8, border:"1px solid #fca5a5", background:"#fff5f5",
                      color:"#dc2626", cursor:"pointer", fontWeight:600 }}>
                      {tr?"Temizle":"Clear all"}</button>
                  )
                )}
              </div>
            )}
            ...
            {selectedYil && yilData && (
              <div style={{ marginBottom:20 }}>
                <div style={{ fontSize:"0.75rem", fontWeight:600, color:"#888", marginBottom:7,
                  textTransform:"uppercase", letterSpacing:"0.05em",
                ...
                <div style={{ border:"1px solid #f0ece8", borderRadius:10, overflow:"hidden", maxHeight:340, overflowY:"auto" }}>
                  {yilData.yariyillar.map(yariyil => (
                    <div key={yariyil.yariyil}>
                      <div style={{ padding:"5px 12px", background:"#f5f0ec", fontSize:"0.71rem",
                        fontWeight:700, color:"#7a1f2b", letterSpacing:"0.04em",
                        borderBottom:"1px solid #ede8e3" }}>
                        {yariyil.yariyil_adi}
                      </div>
                      {(yariyil.dersler||[]).map((d,di) => {
                        const isLast = di===yariyil.dersler.length-1;
                        if (d.secmeli_grup) return <SecmeliGrupRow key={di} grup={d} courses={courses}
                        ...
                        return <CourseRow key={di} ders={d} catalogEntry={findInCatalog(d,courses)}
                          isLast={isLast} tr={tr} done={done} onToggle={toggleDone} viewFilter={viewFilter} />;
                      })}
                    </div>
                  ))}
                ...
                <div style={{ fontSize:"0.71rem", color:"#aaa", marginTop:6 }}>
                  {tr?"☑ tıkla → alındı işaretle · ● yeşil = katalogda var"
                     :"☑ click → mark done · ● green = in catalog"}
                </div>
              </div>
            )}

            {/* CENG ders havuzu (sadece cengCourses yüklüyse) */}
            {cengCourses && (
              <div>
                ...
                <div style={{ border:"1px solid #f0ece8", borderRadius:10, overflow:"hidden", maxHeight:280, overflowY:"auto" }}>
                  ...
                  {tabCourseList.map((c,i) => {
                    if (c._isHeader) return (
                      <div key={i} style={{ padding:"7px 12px", background:"#f9f6f3", fontSize:"0.71rem",
                        fontWeight:600, color:"#7a5c1f", borderBottom:"1px solid #f0ece8" }}>{c.label}</div>
                    );
                    const ders = { kod:c.courseCode||c.kod, ad:c.courseName||c.ad,
        ...
        {/* Alt buton */}
        <div style={{ padding:"12px 20px", borderTop:"1px solid #f0ece8",
          display:"flex", gap:10, justifyContent:"flex-end" }}>
          <button onClick={onClose} style={{ padding:"9px 18px", borderRadius:8,
            border:"1px solid #e5e0da", background:"#fff", color:"#555",
          ...
          <button onClick={handleApply} disabled={!matchedYilDersleri.length} style={{
            ...
            fontSize:"0.85rem",
            cursor: matchedYilDersleri.length?"pointer":"not-allowed", fontWeight:700 }}>
            {selectedYil
