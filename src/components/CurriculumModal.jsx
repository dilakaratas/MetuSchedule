...
// ─── Mühendislik Fakültesi JSON dosyası ──────────────────────────────────────
// Önce yeni scraper çıktısını (metu_engineering_catalog.json) dener,
// bulamazsa eski formatı (metu_eng_faculty_mufredat.json) kullanır.
// İkisi de public/ klasöründe bulunabilir.
const ENG_FILES = [
  "/metu_engineering_catalog.json",      // yeni scraper: { programs: [...] }
  "/metu_eng_faculty_mufredat.json",     // eski scraper: { bolumler: [...] }
];

// ─── Yıl / dönem eşleme tabloları ───────────────────────────────────────────
const YEAR_TO_NUM = {
...
// ─── Yeni scraper JSON → iç yapı dönüşümü ───────────────────────────────────
// Yeni: { programs: [ { program_id, curriculum: [{ year, semester, courses[] }] } ] }
...
// ─── Eski scraper JSON → iç yapı dönüşümü ───────────────────────────────────
// Eski: { bolumler: [ { prog_id, mufredat: [ { yil, yil_adi, yariyillar[] } ] } ] }
// Bu format zaten iç yapıyla aynı — sadece normalize et.
function convertOldCatalog(bolum) {
  const yariyilList = bolum.mufredat || [];
  // yariyillar direkt içinde mi yoksa yil>yariyillar hiyerarşisinde mi?
  // eski engineering_curr.py çıktısı: mufredat = [ { yil, yil_adi?, yariyillar: [...] } ]
