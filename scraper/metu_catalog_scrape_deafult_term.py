...
def post_html_with_retry(session: requests.Session, url: str, payload: dict, retries: int = REQUEST_RETRIES) -> str:
    last_error = None
...
def parse_course_detail(course_detail_html: str):
    ...
    for section_input in section_inputs:
        ...
        # Aynı satırda saat varsa al
        section["times"].extend(extract_times_from_cells(cells))

        # Section satırından sonraki tr'lerde saatler olabilir
...
def normalize_course(parsed_course, fallback_course_code=None):
    ...
    fallback_course_code sadece debug için tutulur; ana courseCode parse edilen olmalı.
    """

    return {
        "courseCode": parsed_course.get("courseCode"),
...
def main():
    ...
    current_term = detect_current_term(options)

    if not current_term:
        raise ValueError("Current term bulunamadı. Sayfa yapısı değişmiş olabilir.")

    print(f"Bulunan current term: {current_term}")

    departments = extract_departments(options, current_term)
    ...
    for index, department in enumerate(selected_departments, start=1):
        ...
        # Her department sonunda ara kayıt alıyoruz.
